...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function color3(hexOrColor) {
    // Accept either a hex string or a BABYLON.Color3
  ...
  function createBoxMaterial(scene, colorInput, texturePath = null, size = 1, uniqueName = "") {
    // colorInput can be hex string or BABYLON.Color3
    ...
    // Slightly tuned specular for nicer lighting
  ...
  /* ============================================================
     RAMP CREATOR - extruded wedge with full-sided material
     (MultiMaterial doesn't work reliably with ExtrudeShape,
      so use one StandardMaterial with backFaceCulling=false)
  ============================================================ */
  function makeTriWedge(scene, name, width = 10, height = 6, depth = 8, pos = {x:0, y:0, z:0}, rotY = 0) {
    ...
    // single StandardMaterial that renders both sides (full faces)
    const mat = createBoxMaterial(scene, "#A6A6A6", null, 1, name);
    mat.backFaceCulling = false; // ensures both sides are visible
    mat.specularPower = 32;
    mat.emissiveColor = new BABYLON.Color3(0,0,0); // small emissive offset left at 0
    ramp.material = mat;
  ...
  /* ============================================================
     SCATTER OBJECTS (TREES, CUBES) - generic helper
  ============================================================ */
  ...
  function createScene() {
    ...
    // physics plugin - ensure cannon.js is loaded in your HTML
    ...
    const groundTopMat = createBoxMaterial(scene, SETTINGS.groundTopColor, null, 1, "groundTop");
    const groundSideMat = createBoxMaterial(scene, SETTINGS.groundSideColor, null, 1, "groundSide");

    const groundMM = new BABYLON.MultiMaterial("groundMM", scene);
    groundMM.subMaterials = [groundSideMat, groundSideMat, groundSideMat, groundSideMat, groundTopMat, groundSideMat];
    ...
    const waterVolMat = createBoxMaterial(scene, SETTINGS.waterColor, null, 1, "waterVol");
    ...
    ramps.forEach((ramp,i) => {
      ...
      tor.material = createBoxMaterial(scene, "#E5B300", null, 1, `ringMat${i}`);
    });
    ...
    const trees = scatterObjects(scene, SETTINGS.treeCount, (x, z, objs) => {
      ...
      trunk.material = createBoxMaterial(scene, "#59320C", null, 1, `trunkMat${id}`);
      ...
      leaf.material = createBoxMaterial(scene, SETTINGS.groundTopColor, null, 1, `leafMat${id}`);

      // group: return trunk (we'll reflect trunks; optional to reflect leaves too below)
      return trunk;
    });
    ...
    const cubes = scatterObjects(scene, SETTINGS.cubeCount, (x,z,objs)=>{
      ...
      // top (green)
      const topMat = createBoxMaterial(scene, SETTINGS.groundTopColor, null, 1, `cubeTop${id}`);

      // side: either coat texture or random color
      let sideMat;
      if (coatTex) {
        sideMat = new BABYLON.StandardMaterial(`sideMat${id}`, scene);
        sideMat.diffuseTexture = coatTex.clone();
        sideMat.specularPower = 8;
      } else {
        sideMat = new BABYLON.StandardMaterial(`sideMat${id}`, scene);
      ...
      // MultiMaterial on box order: [front, back, right, left, top, bottom]
    ...
    /* ---------------- CAMERA ---------------- */
    const camera = new BABYLON.ArcRotateCamera("cam", Math.PI/2, Math.PI/3, 220, BABYLON.Vector3.Zero(), scene);
    camera.attachControl(canvas, true);

    /* ---------------- WATER REFLECTION (after objects exist) ---------------- */
    // Add relevant meshes to the water material's render list so reflections show
